...
# Input: id, [options]
# Output: dcc.Dropdown
def render_dropdown_format_valued(id, options):
    return dcc.Dropdown(id=id, options=[{'label': i, 'value': (i.replace(" ", "")).lower()} for i in options], value=(str(options[0]).replace(" ", "")).lower(),
        className='card h-100' )
